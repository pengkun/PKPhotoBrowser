...
class PKLoadingTool {
    ...
    static let tool = PKLoadingTool()
    var loadingView: PKLoadingView?                   // window上的弹窗
    var loadingVCArray: [UIViewController] = []   // 记录正在显示loading视图的 vc
}

// window 级别的弹窗
extension PKLoadingTool {
    static func show() {
        guard self.tool.loadingView == nil else {return}
        guard let window = UIApplication.shared.delegate?.window else {return}
        guard let win = window else {return}
        
        self.tool.loadingView = PKLoadingView.addTo(win)
    }
...
extension UIViewController {
    // loading视图
    private var ldt_loadingView: PKLoadingView? {
        get {return objc_getAssociatedObject(self, &kLoadingViewKey) as? PKLoadingView}
    ...
    func ldt_loadingCountAdd() {
        ...
        debugPrint("\(self) add loadingCount \(count)")
    }
    func ldt_loadingCountReduce() {
        ...
        debugPrint("\(self) reduce loadingCount \(count)")
    }
