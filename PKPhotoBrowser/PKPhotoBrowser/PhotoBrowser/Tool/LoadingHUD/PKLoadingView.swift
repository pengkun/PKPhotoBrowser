...
//        self.indicatorView.color = UIColor.selectedColor
        self.indicatorView.center = CGPoint(x: ceil(width*0.5), y: ceil(height*0.5))
        
        self.titleLabel.text = "loading"
        self.titleLabel.textColor = .white
        self.titleLabel.font = UIFont.systemFont(ofSize: 18)
        self.titleLabel.frame = CGRect(x: 0, y: width-40, width: width, height: 0)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        let frame = self.indicatorContentView.frame
        debugPrint("\(self.frame)  \(frame)")
    }
}
extension PKLoadingView {
    @discardableResult
    static func addTo(_ superView: UIView) -> PKLoadingView {
        let loadingView = PKLoadingView()
